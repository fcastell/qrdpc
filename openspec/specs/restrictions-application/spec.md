# restrictions-application Specification

## Purpose
Defines how a decoded QR payload becomes an applied Android Managed Configuration:
validation against `qr-payload-format`, conversion to `android.os.Bundle`, a mandatory
confirmation step, and applying via `DevicePolicyManager`'s delegated
`setApplicationRestrictions` call with distinct, actionable error states.
