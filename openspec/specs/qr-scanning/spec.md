# qr-scanning Specification

## Purpose
Defines the camera-based QR capture in QrDPC's Android app: CameraX + ML Kit Barcode
Scanning (QR-only), explicit UTF-8 decoding with BOM stripping per `qr-payload-format`,
and pause/resume behavior around a detection.
