...
## Purpose
Defines the JSON contract encoded in a QrDPC QR code — target package name plus a
typed restrictions list — shared by the web-based generator and any future
Android-side decoder, including the QR transport-level UTF-8 BOM convention needed for
non-ASCII values to round-trip correctly across real-world scanners.
