...
QrDPC's differences from `rms-mobile-app`:
...
- No existing signing keystore or GitHub secrets — these need to be created from scratch
  for QrDPC specifically, not reused from `rms-mobile-app` (separate app, separate identity,
  separate blast radius if compromised).

## Goals / Non-Goals

**Goals:**
- Bring QrDPC's CI/tooling/workflow baseline to parity with `rms-mobile-app` for the parts
...
- Keep local development frictionless: no secrets required to build/run locally; the
  release signing config activates only in CI, exactly like `rms-mobile-app`.
...
- No Play Store publishing — "release" here means a signed APK attached to a GitHub
  Release, matching `rms-mobile-app`'s current scope exactly.
...
- **Single-module tooling wiring**: apply the `detekt` and `spotless` plugins directly in
  ...
  `rms-mobile-app`. Versions (`detekt` 1.23.8, `spotless` 8.8.0, `ktlint` 1.8.0) copied
...
- **lefthook over a Gradle-only check**: keep the same pre-commit hook shape as
  `rms-mobile-app` (`spotless-apply` running `./gradlew spotlessApply` with
...
- **CI workflow structure mirrors `rms-mobile-app`'s `ci.yml`** (fail-fast ordering:
  ...
  `rms-mobile-app` (doc/spec-only changes don't need a full Android build).
- **New `GRADLE_ENCRYPTION_KEY` secret dedicated to QrDPC** (`openssl rand -base64 32`),
  not reused from `rms-mobile-app` — each repo's Gradle configuration-cache encryption key
  stays independent.
- **New signing keystore dedicated to QrDPC**, generated locally with `keytool`
  specifically for this app (distinct identity from `rms-mobile-app`'s), stored only as a
  ...
  `signingConfig` pattern as `rms-mobile-app`: present only when
...
- **`versionCode`/`versionName` sourced from Gradle properties** (`-PversionName`/
  ...
  builds — same mechanism as `rms-mobile-app`, so the release job can derive them from the
...
- [`ANDROID_KEYSTORE_BASE64` accidentally unset/removed from GitHub secrets] → Release job
  ...
  `rms-mobile-app`, inherited intentionally for consistency). Mitigation: the
...
- [Pinned third-party Action SHAs (`gradle/actions/setup-gradle`,
  ...
  `rms-mobile-app` already accepted; mitigation is periodic manual review, not automated
  here.
