...
`a-sibling-project` (multi-module REDACTED-PROJECT project, same maintainer, same core toolchain
...
signed-release job. QrDPC currently has none of this: no `.github/workflows/`, no linting
or formatting enforcement, no pre-commit hook, and no release process beyond manually
building `:app:installDebug` locally. This design ports the reusable parts, adapted to
QrDPC's differences from `a-sibling-project`:

- Single `:app` Gradle module (no `core`/`feature` module split), so tooling wiring is
  simpler — no per-module repetition needed.
- No dedicated target hardware (no Zebra/Sunmi equivalent) — device verification stays
  device-agnostic.
...
- No existing signing keystore or GitHub secrets — these need to be created from scratch
  for QrDPC specifically, not reused from `a-sibling-project` (separate app, separate identity,
  separate blast radius if compromised).

## Goals / Non-Goals

**Goals:**
- Bring QrDPC's CI/tooling/workflow baseline to parity with `a-sibling-project` for the parts
  that generalize: formatting/lint/static-analysis/test/build on every PR, a documented
  Claude Code workflow, and a reproducible signed-release process.
- Keep local development frictionless: no secrets required to build/run locally; the
  release signing config activates only in CI, exactly like `a-sibling-project`.
...
- No tester installation guide (`docs/guide-installation-testeurs.md` equivalent) — QrDPC
  has no internal tester distribution process yet; the release job's notes stay a short
  fixed English blurb plus `--generate-notes`, with no guide link, until/unless that need
  arises.
- No Play Store publishing — "release" here means a signed APK attached to a GitHub
  Release, matching `a-sibling-project`'s current scope exactly.
...
- **Single-module tooling wiring**: apply the `detekt` and `spotless` plugins directly in
  ...
  (`FunctionNaming` ignored for `@Composable`) and `.editorconfig` entry as
  `a-sibling-project`. Versions (`detekt` 1.23.8, `spotless` 8.8.0, `ktlint` 1.8.0) copied
  into `gradle/libs.versions.toml` to match the already-shared AGP/Kotlin versions.
- **lefthook over a Gradle-only check**: keep the same pre-commit hook shape as
  `a-sibling-project` (`spotless-apply` running `./gradlew spotlessApply` with
  `stage_fixed: true`) rather than only relying on CI to catch formatting — catches issues
  before push, consistent with the sibling project.
- **CI workflow structure mirrors `a-sibling-project`'s `ci.yml`** (fail-fast ordering:
  ...
  `paths-ignore` covers `openspec/**`, `docs/**`, `**/*.md`, same rationale as
  `a-sibling-project` (doc/spec-only changes don't need a full Android build).
- **New `GRADLE_ENCRYPTION_KEY` secret dedicated to QrDPC** (`openssl rand -base64 32`),
  not reused from `a-sibling-project` — each repo's Gradle configuration-cache encryption key
  stays independent.
- **New signing keystore dedicated to QrDPC**, generated locally with `keytool`
  specifically for this app (distinct identity from `a-sibling-project`'s), stored only as a
  base64-encoded `ANDROID_KEYSTORE_BASE64` GitHub Actions secret (plus
  ...
  `signingConfig` pattern as `a-sibling-project`: present only when
...
- **`versionCode`/`versionName` sourced from Gradle properties** (`-PversionName`/
  ...
  builds — same mechanism as `a-sibling-project`, so the release job can derive them from the
...
- **Release notes in English**: fixed text along the lines of "QrDPC test build." plus
  ...
  `qrdpc-vX.Y.Z.apk`, matching the `redacted-project-vX.Y.Z.apk` naming convention translated to
  this app's name.
- **`verify-on-device` generalized**: keep the build → install → cold-start →
  ...
  alone" rule, but drop the Zebra/Sunmi hardware constraint entirely — any connected,
...
- **`git-pr-workflow` and `session-wrapup` ported near-verbatim**, translated to English,
  with `redacted-project`/`com.example.redacted` references replaced by `qrdpc`/
...
- [`ANDROID_KEYSTORE_BASE64` accidentally unset/removed from GitHub secrets] → Release job
  silently produces an *unsigned* APK instead of failing loudly (same behavior as
  `a-sibling-project`, inherited intentionally for consistency). Mitigation: the
...
- [Pinned third-party Action SHAs (`gradle/actions/setup-gradle`,
  ...
  `a-sibling-project` already accepted; mitigation is periodic manual review, not automated
  here.
