...
The sibling project `a-sibling-project` (same toolchain: AGP 9.3.1, Kotlin 2.2.20, same
...
documentation/commits in English (unlike `a-sibling-project`, which is French — that
...
- Add code quality tooling: `detekt` (static analysis) and `spotless`/`ktlint` (formatting),
  ...
  entry, matching `a-sibling-project`'s configuration.
...
- Add a conditional release `signingConfig` to `app/build.gradle.kts`: active only when
  ...
  `a-sibling-project`'s approach exactly.
